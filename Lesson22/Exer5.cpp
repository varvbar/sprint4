...
public:
    ...
    IteratorRange(const Iterators begin, const Iterators end, const size_t page_size)
...
ostream& operator<< (ostream& out, const IteratorRange<Iterator>& iterator_range){
...
{
public:
    Paginator (const Iterator begin_doc, const Iterator end_doc, const size_t page_size)
    {
        size_t document_size = distance(begin_doc, end_doc);
        ...
        if(distance(page_begin, end_doc) > 0){
            size_t dist = distance(page_begin, end_doc);
...
public:
    ...
    vector<Document> AddFindRequest(const string& raw_query, DocumentPredicate document_predicate) {
        // напишите реализацию
        const vector<Document> result_find = search_server_.FindTopDocuments(raw_query, document_predicate);
        AddRequest(static_cast<int> (result_find.size()));
        return result_find;
    }
    vector<Document> AddFindRequest(const string& raw_query, DocumentStatus status) {
        // напишите реализацию
        const vector<Document> result_find = search_server_.FindTopDocuments(raw_query, status);
        AddRequest(static_cast<int> (result_find.size()));
        return result_find;
    }
    vector<Document> AddFindRequest(const string& raw_query) {
        // напишите реализацию
        const vector<Document> result_find = search_server_.FindTopDocuments(raw_query);
        AddRequest(static_cast<int> (result_find.size()));
        return result_find;
    }
